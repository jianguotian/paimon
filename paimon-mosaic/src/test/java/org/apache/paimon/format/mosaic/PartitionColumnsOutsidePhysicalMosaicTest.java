...
class PartitionColumnsOutsidePhysicalMosaicTest {
    ...
    @Test
    void testBusinessProjectionPreservesArrowBundle() throws Exception {
        assertBusinessOnlyArrowRead(table.newReadBuilder().withProjection(new int[] {0, 1}));
    }

    @Test
    void testBusinessReadTypePreservesArrowBundle() throws Exception {
        RowType businessType = table.rowType().project(BUSINESS_COLUMNS);
        assertBusinessOnlyArrowRead(table.newReadBuilder().withReadType(businessType));
    }
    ...
    private void assertBusinessOnlyArrowRead(ReadBuilder readBuilder) throws Exception {
        ...
        try (RecordReader<InternalRow> reader = readBuilder.newRead().createReader(split)) {
            ...
            assertThat(bundle.getRowType().getFieldNames())
                    .containsExactlyElementsOf(BUSINESS_COLUMNS);
